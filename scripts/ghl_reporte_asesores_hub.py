#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Genera la hoja unificada "Asesores comerciales": contenedor con dos sub-pestañas
que carga asesores-gestion.html (auditoría y gestión de asesores) y
conversaciones-full.html (análisis de conversaciones de WhatsApp) en un iframe del
mismo sitio, ocultando el encabezado interno de cada una (mismo patrón que la hoja
Estrategia comercial). También genera el redirect de auditoria-whatsapp.html →
asesores.html?tab=conversaciones para no romper marcadores viejos."""
...
<div><h1>Asesores comerciales</h1>
<p>CRM comercial (solo lectura) · Gestión y auditoría de asesores + conversaciones + pérdida de leads · Corte: {CORTE}</p></div>
</div></header>
...
<div class="strip"></div>
<div class="subtabs">
<button class="subtab on" id="t-ges" data-src="asesores-gestion.html">👤 Gestión de asesores</button>
<button class="subtab" id="t-conv" data-src="conversaciones-full.html">💬 Análisis de conversaciones</button>
<button class="subtab" id="t-desc" data-src="descartados-full.html">🗑 Pérdida de leads</button>
</div>
<div class="frwrap"><iframe id="fr" src="asesores-gestion.html" title="Contenido de asesores comerciales"></iframe></div>
<script>
const fr = document.getElementById('fr');
const tabs = [...document.querySelectorAll('.subtab')];
function abrir(tab) {{
  tabs.forEach(t => t.classList.toggle('on', t === tab));
  /* location.replace: cambia el contenido SIN crear entradas de historial */
...
fr.addEventListener('load', () => {{
  ...
  }} catch (e) {{ /* file://: solo estético */ }}
}});
const _tab = new URLSearchParams(location.search).get('tab');
if (_tab === 'conversaciones') abrir(document.getElementById('t-conv'));
if (_tab === 'descartados') abrir(document.getElementById('t-desc'));
</script>
</body></html>'''

out = ROOT / f'asesores-hub-pfs-{_hoy:%Y-%m-%d}.html'
out.write_text(HTML, encoding='utf-8')

REDIR = '''<!DOCTYPE html><html lang="es"><head><meta charset="utf-8">
<meta http-equiv="refresh" content="0;url=asesores.html?tab=conversaciones">
<title>Conversaciones</title></head>
<body style="font-family:system-ui;padding:30px;color:#152238">El análisis de conversaciones ahora vive en la hoja
<a href="asesores.html?tab=conversaciones">Asesores comerciales</a>. Redirigiendo…</body></html>'''
red = ROOT / f'wa-redirect-pfs-{_hoy:%Y-%m-%d}.html'
red.write_text(REDIR, encoding='utf-8')
REDIR2 = '''<!DOCTYPE html><html lang="es"><head><meta charset="utf-8">
<meta http-equiv="refresh" content="0;url=asesores.html?tab=descartados">
<title>Pérdida de leads</title></head>
<body style="font-family:system-ui;padding:30px;color:#152238">El análisis de pérdida de leads ahora vive en la hoja
<a href="asesores.html?tab=descartados">Asesores comerciales</a>. Redirigiendo…</body></html>'''
red2 = ROOT / f'desc-redirect-pfs-{_hoy:%Y-%m-%d}.html'
red2.write_text(REDIR2, encoding='utf-8')
print('OK ->', out)
print('OK ->', red)
print('OK ->', red2)
