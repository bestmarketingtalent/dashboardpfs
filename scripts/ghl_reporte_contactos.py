...
['f-d1','f-d2'].forEach(id => document.getElementById(id).addEventListener('change', () => {{ apply(); renderScat(); }}));
...
(function initScatCtrl() {{
  ...
  const opts = [['hot', '🔥 Calientes (score ≥ 55)']]
    .concat(STATUS_ORDER.filter(s => D.status.indexOf(s) >= 0)
            .map(s => [String(D.status.indexOf(s)), s]));
...
function scatData() {{
  ...
  const F = new Map();
  D.rows.filter(inFecha).forEach(r => {{
    let o = F.get(r[7]);
    if (!o) {{ o = {{ t: 0, st: {{}}, hotSt: {{}} }}; F.set(r[7], o); }}
    o.t++;
    o.st[r[4]] = (o.st[r[4]] || 0) + 1;
    if (r[9] >= 55) o.hotSt[r[4]] = (o.hotSt[r[4]] || 0) + 1;
  }});
  const pts = [];
  F.forEach((o, fi) => {{
    if (o.t < minN) return;
...
function arranque() {{
  ...
  document.getElementById('sc-min').value = '10';
  reset();
}}
arranque();
